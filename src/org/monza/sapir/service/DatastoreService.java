...
public class DatastoreService {
	...
	public Portfolio loadPortfolilo() {
		...
		try {
			...
			if(symbolArray != null) {
				...
				portfolio = new Portfolio(new StockStatus[MAX_PROTFOLIO_SIZE], 0, "UNKNOWE",0);
			}
	...
	public void updatePortfolio(Portfolio portfolio) {
		...
		updateStocks(Lists.newArrayList(portfolio.getStock()));
	}
	...
	private StockStatus fromStockEntry(Entity stockEntity) {
		...
		ret.setStockQuntity(quantity.intValue());
	...
	private Entity stockToEntity(StockStatus stock) {
		...
		entity.setProperty(STOCK_QUANTITY, stock.getStockQuntity());
	...
	private void updateStock(StockStatus stockStatus) {
		...
		for (Entity entity : pq.asIterable()) {
			entity.setProperty(STOCK_QUANTITY, stockStatus.getStockQuntity());
